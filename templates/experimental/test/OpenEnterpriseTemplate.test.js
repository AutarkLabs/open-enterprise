...
contract('OpenEnterpriseTemplate', ([ owner, member1, member2, member3 ]) => {
  ...
  before('deploy required contract dependencies', async () => {
    ...
    // assign 'open.aragonpm.eth' to apmFactory
    await ens.setSubnodeOwner(openTldHash, openLabelHash, apmFactory.address)
    ...
    const openApmReceipt = await apmFactory.newAPM(openTldHash, openLabelHash, owner)
    ...
    const openApm = getContract('APMRegistry').at(getReceipt(openApmReceipt, 'DeployAPM', 'apm'))
    console.log('       Deployed Open APM at', openApm.address)
    ...
    for (const { name, contractName } of APPS) {
      ...
      if (name.includes('hatch')) {
        ...
        await openApm.newRepoWithVersion(name.replace('.open',''), owner, [ 1, 0, 0 ], appBase.address, '')
      } else {
