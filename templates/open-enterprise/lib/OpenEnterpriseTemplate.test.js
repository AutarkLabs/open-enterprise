    ...
    context('when the creation succeeds', () => {
      ...
      const createDAO = (useDiscussions = false, periods = [ 0, 0 ]) => {
        before('create open enterprise entity', async () => {
          daoID = randomId()
          baseDAO = await template.newTokenAndInstance(TOKEN_NAME, TOKEN_SYMBOL, daoID, MEMBERS, STAKES, VOTING_SETTINGS, periods[0], TOKEN_TRANSFERABLE)
      ...
      xcontext('when requesting a custom finance period', () => {
        ...
        xcontext('when requesting a vault app', () => {
