    ...
    const issuesTableRows = visibleIssues.map((issue, index) => {
      ...
      return (
        <TableRow key={issue.node.id}>
          <TableCell>
            <CheckboxInput
              isChecked={issue.node.id in selectedIssues}
              onClick={checkboxHandler}
            />
          </TableCell>
          <TableCell>
            <Text>
              {issue.node.repository.name} #{issue.node.number}
            </Text>
          </TableCell>
          <TableCell>
            <SafeLink
      ...
      <IssuesMain>
        <Filters>
          ...
          <Filter>
            ...
            <DropDown
              items={milestonesNames}
              active={activeMilestoneNameIndex}
              onChange={this.handleMilestoneSelect}
            />
          </Filter>
          <Filter>
            <DropDownLabel>Bounty</DropDownLabel>
            <DropDown
              items={bountiesFilterItems}
              active={activeBountyNameIndex}
              onChange={this.handleBountySelect}
            />
          </Filter>
          <Button
            mode={
                    ...
                    <BountyAssignForm>
                      ...
                      <div style={{ marginLeft: '10px' }}>
                        <DropDown
                          ...
                          onChange={handleChangeIssueBounty}
                        />
                      </div>
                    </BountyAssignForm>
                  </BountyTableCell>
