...
contract AllocationsTemplate is KitBase {
    ...
    function createTPSApps (
        ...
        dotVoting = DotVotingApp(dao.newAppInstance(apps[1], latestVersionAppBase(apps[1])));
        allocations = Allocations(dao.newAppInstance(apps[2], latestVersionAppBase(apps[2])));
    ...
    ) internal
    {
        ...
        acl.createPermission(ANY_ENTITY, allocations, allocations.CREATE_ALLOCATION_ROLE(), voting);
    ...
    function createDiscussionApp(
        ...
        ACL acl,
        Kernel dao
    ) internal returns (DiscussionApp app) {
