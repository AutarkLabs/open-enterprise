...
context('Aragon', () => {
  ...
  context('Rewards', () => {
    ...
    it('creates New Reward (Merit)', () => {
      ...
      cy.getButton('New Reward').click().wait(1000)
      .appFind('input[name="description"]').type("Merit Reward")
      .appFind('input[name="amount"]').type(3)
      .appFind('div[data-e2e="reward-amount-currency"]').click('right')
      .within(($el) => {
        cy.contains('div','autark').click().wait(1000)
      })

      cy.appContains('div','Select a token').click().wait(1000)
      .appFind('span[data-e2e-token="autark"]').click({force: true}).wait(1000)
      .appFind('input[name="periodEnd"]').type(tomorrow)
      .appFind('input[name="periodEnd"]').trigger('keydown', {
        keyCode: 13,
        which: 13,
        })
      .getButton('Submit Reward').click().wait(1000)
    ...
    it('creates New Reward (Dividend)', () => {
      ...
      cy.getButton('New Reward').click().wait(1000)
      .appFind('input[name="description"]').type("Dividend Reward")
      .appFind('input[name="amount"]').type(4)
      .appFind('div[data-e2e="reward-amount-currency"]').click('right')
      .within(($el) => {
        cy.contains('div','autark').click().wait(1000)
      })

      cy.appContains('div','Merit Reward').click().wait(1000)
      .appContains('div','Dividend').click().wait(1000)

      cy.appContains('div','Select a token').click().wait(1000)
      .appFind('span[data-e2e-token="autark"]').click({force: true}).wait(1000)
      .appFind('input[name="dateEnd"]').type(tomorrow)
      .appFind('input[name="dateEnd"]').trigger('keydown', {
        keyCode: 13,
        which: 13,
        })
      .getButton('Submit Reward').click().wait(1000)
    ...
    it('shows New Reward in Panel', () => {
      cy.contains('span', 'Rewards', { timeout: 150000 }).click().wait(4000)
      cy.appFind('span[data-e2e-reward-description="0"]', 'Some Reward').click().wait(500)
      cy.appContains('span[data-e2e-reward-badge-amount="0"]', 3)
